<script>
    ...
    const handleRevitAudit = async () => {
        // Use the standard browser-based audit (same as home tab)
        // Revit integration is only for element selection, not validation
        try {
            isAuditing = true;
            await runAudit();
            success('Audit completed successfully');
        ...
        <div class="content-body">
            ...
            {:else if activeTab === 'revit'}
                ...
                {:else}
                    <div class="section">
                        <h3>Connection Status</h3>
                        <div class="connection-status {Revit.connected ? 'connected' : 'disconnected'}">
                            <div class="status-indicator"></div>
                            <span class="status-text">
                                {Revit.connected ? 'Connected' : 'Not Connected'}
                            </span>
                            {#if Revit.apiUrl}
                                <span class="status-details">{Revit.apiUrl}</span>
                            {/if}
                        </div>
                        
                        <button class="load-btn" onclick={Revit.connected ? disconnectRevit : connectRevit} disabled={Revit.loading}>
                            {#if Revit.loading}
                                <svg class="spinner" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M21 12a9 9 0 11-6.219-8.56"/>
                                </svg>
                                Connecting...
                            {:else if Revit.connected}
                                <XIcon size={18} />
                                Disconnect
                            {:else}
                                <LinkIcon size={18} />
                    ...
                    {#if Revit.connected}
                        <div class="section">
                            <h3>Load IFC File</h3>
                            <div 
                                class="drop-zone {revitDragOver ? 'drag-over' : ''}"
                                ondrop={handleRevitDrop}
                                ondragover={handleRevitDragOver}
                                ondragleave={handleRevitDragLeave}
                            ...
                            >
                                ...
                                <p class="drop-zone-text">Drag & drop IFC file here</p>
...
<style>
    ...
    /* Revit Integration Drop Zone */
    .drop-zone {
        border: 2px dashed #5555556e;
