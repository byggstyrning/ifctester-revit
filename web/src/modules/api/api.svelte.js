...
export async function clearAllModels() {
    try {
        // Unload all models from WASM
        for (const model of IFCModels.models) {
            try {
                await wasm.unloadIfc(model.id);
            } catch (error) {
                console.warn(`Failed to unload model ${model.id}:`, error);
            }
        }
        
        // Clear models and audits arrays
        IFCModels.models = [];
        IFCModels.audits = [];
        
        console.log('All IFC models cleared');
    } catch (error) {
