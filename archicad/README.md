...
2. **Configures CMake** (if `Build` doesn't exist - generates Visual Studio project files)
...
**Note:** The `Build/` directory is gitignored because it contains generated files. The build script automatically runs CMake configure if needed. All source files (`CMakeLists.txt`, source code, resources) are tracked in git.
...
The `Build/` directory is **gitignored** because it contains generated files. Here's how it works:

1. **First build**: The build script detects that `Build/` doesn't exist and automatically runs `cmake configure` to generate it
2. **Subsequent builds**: The script uses the existing `Build/` directory
3. **Clean rebuild**: Delete `Build/` folder and run the build script again - CMake will regenerate everything
...
```
archicad\Build\IfcTesterArchiCAD.sln
```
...
- ❌ `Build/` - Generated files (gitignored, recreated by CMake)
...
- **Build output**: `archicad\Build\Debug\IfcTesterArchiCAD.apx` (or `Release`)
...
1. **Configure CMake** (generates `Build/` directory and `.sln` file):
   ```powershell
   cd archicad
   cmake -B Build -G "Visual Studio 17 2022" -A x64 -DAC_API_DEVKIT_DIR="C:\code\archicad-api\API.Development.Kit.WIN.29.3100"
   ```

2. **Open in Visual Studio**:
   - Open `Build\IfcTesterArchiCAD.sln` in Visual Studio 2022
...
**Note:** The `Build/` directory (including `IfcTesterArchiCAD.sln`) is gitignored because it contains generated files. CMake will recreate everything when you run `cmake configure`. The source files (`CMakeLists.txt`, `CMakeSettings.json`, and all source code) are tracked in git.

#### macOS Build

1. **Configure CMake** (generates `Build/` directory):
   ```bash
   cd archicad
   ...
   **OR using Unix Makefiles:**
   ```bash
   cmake -B Build -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Release -DAC_API_DEVKIT_DIR="$HOME/Library/Application Support/GRAPHISOFT/API Development Kit 29"
   ```

2. **Build the add-on:**
   
   **Using Xcode:**
   - Open `Build/IfcTesterArchiCAD.xcodeproj` in Xcode
...
3. **Verify the build output:**
   ```bash
   ls -la Build/Release/IfcTesterArchiCAD.bundle
   ```
...
The compiled add-on will be located at:
```
archicad/Build/Release/IfcTesterArchiCAD.bundle
```
...
2. **Manual verification:**
   - Build output: `archicad\Build\Release\IfcTesterArchiCAD.apx`
   - Installed location: `%APPDATA%\Graphisoft\ArchiCAD 29\Add-Ons\IfcTesterArchiCAD\IfcTesterArchiCAD.apx`
   ...
   **Issue: Connection Timeout / ERR_SOCKET_NOT_CONNECTED**
   ...
   - **Solution**: 
     ...
     4. Rebuild and reinstall the add-on:
        ...
        cmake --build Build --config Release
        cd ..
        .\scripts\copy-build.ps1 -Configuration Release -ArchiCADVersion 29
        ```
...
The output will be in `Build/Release/IfcTesterArchiCAD.apx`.
